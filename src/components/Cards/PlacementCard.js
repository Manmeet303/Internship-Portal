...
const PlacementCard = ({ props, type }) => {
  ...
  const showApplicantsHandler = () => {
    ...
    } else history.push(`/Company/Placement/${props._id}/Applicants`);
  };
  const updateHandler = (e) => {
    e.preventDefault();
    history.push("/Company/Placement/Update/" + props._id);
  };
  const removeHandler = async (e) => {
    e.preventDefault();
    await dispatch(removePlacement(props._id));
    dispatch(getPlacement({ companyId: props.companyId }));
  };
  const applyHandler = async (e) => {
    e.preventDefault();
    if (!isApplied) {
      const checkApplied = await dispatch(applyPlacement(props._id));
      ...
      const isWithdraw = await dispatch(
        withdrawApplication({ id: placementId, workType })
      );
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <th colSpan={1}>Required CGPA</th>
            <td colSpan={1}>{props?.cgpaRequirement}</td>
          </tr>
          <tr>
